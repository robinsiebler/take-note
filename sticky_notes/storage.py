...
def _default_path() -> Path:
    ...
    return base / "sticky-notes" / "notes.json"


def save_all(notes: list[Note], boards: list[Board], path: Path | None = None) -> None:
...
def load_all(path: Path | None = None) -> tuple[list[Note], list[Board]]:
    path = path or _default_path()
