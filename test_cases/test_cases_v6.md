# Take Note! — Manual Test Cases (v6)

Covers everything merged to `main` as of 2026-07-12. No branch-specific
...
**v6 changes from v5:** the v5 pass found several real bugs, all now
fixed:

- **The Delete Note dialog appeared far from the note it belonged to**
  (§2.7) — a side effect of the v5 width fix: centering was computed
  before the dialog's real (widened) size had actually been laid out.
- **The Font dialog silently stripped bold from mixed-formatting
  selections** (§3.1) — bolding one word, then changing only the size
  on a selection spanning it, wiped bold from that word too. Font…
  now only ever touches family/size; bold/italic/underline/
  strikethrough are left alone (they already have their own
  Ctrl+B/I/U/K shortcuts).
- **The Settings dialog could clip content again** (§9.8) — this time
  from a *saved* size left over from before an earlier content change,
  never re-checked against what the dialog currently needs. Now always
  grows to fit on open, regardless of whether a saved size exists.
...
**Still unresolved — grey/faded text, seen at least twice more during
the v5 pass, cause unknown.** A targeted repro attempt (toggling
underline/italic mid-line, pressing Enter, typing a new line) did not
reproduce it. This is a *different* pattern from the §3.13 bug fixed
last version (that one was a whole word/line; this one was just the
first character of a word) — flagged fresh below as §3.14, a real,
recurring, still-open watch-for, not dismissed.
...
- [x] **1.4** Click the tray icon directly (not right-click) → creates a
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **1.5** Open the Notes Manager and note the **Date Modified** value
...
- [x] **2.6** Header **▲/▼** button (or double-click the header) rolls
  ...
  **with a title set** (§4.5) specifically — the collapsed header should
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **2.7** Header `×` → confirmation dialog **"Move this note to
...
- [x] **3.1** **Font…** opens the native font-family/size/style picker;
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **3.1a** Bold just one word in a sentence (leave the rest plain),
...
- [x] **3.13** Type a line of text, press Enter 5-6 times (leaving
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **3.14** *(Watch-for, still unconfirmed — happened at least twice
...
- [x] **6.2** After sticking, minimizing/restoring the target window
  ...
  - [ ] Fail
  - [x] Pass with Issues
    - When a note is stuck to a window, the note appears in the taskbar
      with the Vorta icon. It also shows up in the task switcher.
      Unsticking the window does not fix the issue, nor does
      hiding/showing it. The only way to fix it is to close/relaunch
      the app.
...
- [x] **8.1** Tray → **Notes Manager…** opens a single window titled
...
- [x] **8.15** Creating/deleting/renaming a note or board elsewhere in
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **8.16** Resize and/or move the Notes Manager window, then quit
  and relaunch the app — it reopens at the exact size/position you left
  it at.
  - [x] Pass
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **8.17** Press the new global hotkey (default `Meta+Alt+B`) from
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **8.18** With the Notes Manager open, check your taskbar and
...
- [x] **8.19** Trash a note (any method above), then click the **Trash**
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **8.20** While viewing Trash, the toolbar's **Delete** button
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **8.21** Select a trashed note and click **Restore** (toolbar or
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **8.22** Trash a note that's attached to a Notepad board, then
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **8.22a** Trash a board-attached note, then delete the *board*
  ...
  - [ ] Fail
  - [x] Pass with Issues
    - See screenshot: the Delete Notepad confirmation dialog appears
      behind the note windows.
- [x] **8.23** Select one or more trashed notes and click **Delete
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **8.24** Double-clicking a row while viewing Trash does nothing
...
- [x] **9.2** **New notes stay on top by default** checkbox controls new
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **9.2a** **Default note size** and **Default notepad size**
...
- [x] **9.8** Move/resize the Settings dialog, click **OK**, reopen it —
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **9.8b** Shrink the Settings dialog as small as it'll go, click
...
- [x] **9.8a** The Hotkey tab's five sections don't all fit without
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **9.8b** A grey hint at the top of the Hotkey tab reads "If
  ...
  *customized* global shortcuts to a file we could read, not its large
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **9.9** Current global hotkey (blank for the three bulk-action
...
- [x] **9.10** Clicking **OK** actually commits a changed hotkey — the
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **9.10a** Set any field to the exact combo currently shown in a
  ...
  **Apply**) without changing either field — a dialog box names both
  ...
  - [ ] Pass
  - [x] Fail
    - Originally found live: setting Show/Hide to the same combo as New
      Note (Meta+Alt+N) let OK commit it with no warning, and pressing
      the physical combo afterward just fired New Note — no conflict
      detection anywhere. Fixed mid-pass (OK/Apply now refuse to commit
      two hotkey fields set to the same combo, matching the wording
      above) — re-verify on the next pass.
  - [ ] Pass with Issues
- [x] **9.10b** Click **Clear** next to any field — it empties
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **9.10c** With a field cleared, click that section's **Test** —
  reports **"Enter a key combination first"** rather than erroring or
  silently doing nothing.
  - [x] Pass
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **9.10d** Set and OK a combo for **Show/Hide All Notes**, then
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **9.11** *(Known gap)* This tab only configures these five global
...
- [x] **10.1** **New Note** / **New Notepad** / **Notes Manager…** each
...
- [x] **10.3** One **Show/Hide All Notes** item. With all notes visible,
  ...
  - [ ] Fail
  - [x] Pass with Issues
    - Hide hides all the notes, but when you toggle it again, all the
      notes come back... with a Vorta icon in the taskbar showing them
      all.
...
- [x] **12.6** Right-click a badly garbled "word" with no close match —
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **12.6a** Right-click a misspelled word and click **Ignore** — the
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **12.6b** Right-click a *different* misspelled word and click **Add
...
- [x] **13.6** Tag a couple of different notes (some overlapping tags,
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **13.7** In the Notes Manager's left tree, a **Tags** entry
...
- [x] **13.11** A brand-new, untagged note shows no tag icon anywhere in
  its header — just `+`, roll, lock, `☰`, `×`.
  - [x] Pass
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **13.12** Tag that note (☰ → **Tags…**, type e.g. `work, urgent`,
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **13.13** With the note focused, hover the tag icon — a tooltip
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **13.14** Click the tag icon directly (not the ☰ menu) — it opens
  the same **Tags…** dialog as ☰ → **Tags…**, pre-filled with the
  note's current tags.
  - [x] Pass
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **13.15** From that dialog, clear the tags field entirely and
...
- [x] **13.16** Tag a note, then also lock it (☰ → **Lock Note**, or the
  ...
  - [ ] Fail
  - [ ] Pass with Issues
- [x] **13.17** Tag a note, quit the app, and relaunch it — the tag icon
